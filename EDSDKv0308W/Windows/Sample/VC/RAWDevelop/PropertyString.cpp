...
EdsError CPropertyItem::GetPropertyTitle(EdsUInt32	*ioSize, EdsChar *outTitle)
{
	...
	if(*ioSize>=strlen(m_title))	strncpy_s(outTitle, *ioSize+1, m_title, *ioSize);
	return EDS_ERR_OK;
}
...
EdsError CPropertyItem::GetPropertyValue(EdsUInt32	index, EdsUInt32	*ioSize, EdsChar *outValue)
{
	...
	if(*ioSize>=strlen(m_value[index].c_str()))	strncpy_s(outValue, *ioSize+1, m_value[index].c_str(), *ioSize);
	return EDS_ERR_OK;
}
