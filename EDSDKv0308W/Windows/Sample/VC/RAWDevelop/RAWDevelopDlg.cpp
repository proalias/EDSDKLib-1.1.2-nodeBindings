...
void CRAWDevelopDlg::OnBnClickedOpenFile()
{
	if( m_pDrawImage == NULL )
		return;
	static char BASED_CODE gbszFilter[] = "CR2 Files (*.CR2)|*.CR2|CR3 Files (*.CR3)|*.CR3|CRW Files (*.CRW)|*.CRW|TIF Files (*.TIF)|*.TIF|AllFiles (*.*)|*.*||";
	...
	if( dlg.DoModal() == IDOK )
	{
		...
		err = EdsCreateFileStream(dlg.GetPathName(),kEdsFileCreateDisposition_OpenExisting,kEdsAccess_Read,&StreamRef);
		if( err == EDS_ERR_OK )
		{
			if(m_ImageRef)
			{
